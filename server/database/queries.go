...
const (
	...
	(timezone('UTC', now()), $1, $2, $3, $4, $5, $6, 1, $7, $8, $9, $10) returning id`

	LawyerLoginQ = `SELECT id, password FROM lawyers WHERE email_address = $1`

	CreateMeetingQ = `insert into meetings(created_at, case_id, lawyer_id) values (timezone('UTC', now()), $1, (select lawyer_id from cases where id = $1 limit 1)) returning id`

	DeleteMeetingGPTRespQ = `delete from gpt_resp where meeting_id = $1 returning true`
	DeleteMeetingQ        = `delete from meetings where id = $1 returning true`

	CreateGPTRespQ = `insert into gpt_resp(created_at, meeting_id, questions, summary, points) values (timezone('UTC', now()), $1, $2, $3, $4) returning id`

	AddNotesToMeetingQ = `update meetings set lawyer_notes = $2 where id = $1 returning id`

	GetCaseDetailsQ = `select created_at, client_first_name, client_last_name, phone_number, email_address, type, description, lawyer_id, address_street, address_city, address_state, address_zip, COALESCE(gpt_summary, 'No AI-generated summary yet - will be completed after first meeting.') AS gpt_summary from cases where id=$1`
